...
(function() {
  var App, Config, MONTHS, Message, Modal, Next, Prev, Query, QueryForm, Results, Stats, Summary, Timeline, format_date_range, solr_query_uri,
  ...
  Prev = (function(_super) {
    ...
    Prev.prototype.initialize = function() {
      return App.results.on('change', this.render);
    };

    Prev.prototype.render = function() {
      if (App.results.offset() < App.results.get('count')) {
        return this.$el.html($('#prev-tpl').html());
      } else {
        return this.$el.html("<div></div>");
      }
    };

    Prev.prototype.events = {
      'click': function() {
        if (App.results.offset() < App.results.get('count')) {
          App.query.prev();
        }
  ...
  Next = (function(_super) {
    ...
    Next.prototype.initialize = function() {
      return App.results.on('change', this.render);
    };

    Next.prototype.render = function() {
      if (App.results.get('start') > 0) {
        return this.$el.html($('#next-tpl').html());
      } else {
        return this.$el.html("<div></div>");
      }
    };

    Next.prototype.events = {
      'click': function() {
        if (App.results.get('start') > 0) {
          App.query.next();
        }
  ...
  Timeline = (function(_super) {
    ...
    Timeline.prototype.render = function() {
      var axis, context, data, epsilon, h, padding, r, svg, tip, x, y;
      padding = 25;
      ...
      if (data.length) {
        ...
        }).attr("r", 5).on('mouseover', tip.show).on('mouseout', tip.hide).on('click', tip.hide);
        axis = d3.svg.axis().scale(x).orient("bottom");
